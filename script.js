(function() {
  var clearTooltipDismissedState, closeJobTooltips, dismissJobTooltip, jobTooltipMediaQuery, setJobTooltipState, syncJobTooltips, tooltipCloseButtons, tooltipItems, tooltipTriggers;
  $(".skills-prog li")
    .find(".skills-bar")
    .each(function(i) {
      $(this)
        .find(".bar")
        .delay(i * 150)
        .animate(
          {
            width:
              $(this)
                .parents()
                .attr("data-percent") + "%"
          },
          1000,
          "linear",
          function() {
            return $(this).css({
              "transition-duration": ".5s"
            });
          }
        );
    });

  $(".skills-soft li")
    .find("svg")
    .each(function(i) {
      var c, cbar, circle, percent, r;
      circle = $(this).children(".cbar");
      r = circle.attr("r");
      c = Math.PI * (r * 2);
      percent = $(this)
        .parent()
        .data("percent");
      cbar = (100 - percent) / 100 * c;
      circle.css({
        "stroke-dashoffset": c,
        "stroke-dasharray": c
      });
      circle.delay(i * 150).animate(
        {
          strokeDashoffset: cbar
        },
        1000,
        "linear",
        function() {
          return circle.css({
            "transition-duration": ".3s"
          });
        }
      );
      $(this)
        .siblings("small")
        .prop("Counter", 0)
        .delay(i * 150)
        .animate(
          {
            Counter: percent
          },
          {
            duration: 1000,
            step: function(now) {
              return $(this).text(Math.ceil(now) + "%");
            }
          }
        );
    });

    
  $(".languages li")
  .find(".skills-bar")
  .each(function(i) {
    $(this)
      .find(".bar")
      .delay(i * 150)
      .animate(
        {
          width:
            $(this)
              .parents()
              .attr("data-percent") + "%"
        },
        1000,
        "linear",
        function() {
          return $(this).css({
            "transition-duration": ".5s"
          });
        }
      );
  });

$(".languages li")
  .find("svg")
  .each(function(i) {
    var c, cbar, circle, percent, r;
    circle = $(this).children(".cbar");
    r = circle.attr("r");
    c = Math.PI * (r * 2);
    percent = $(this)
      .parent()
      .data("percent");
    cbar = (100 - percent) / 100 * c;
    circle.css({
      "stroke-dashoffset": c,
      "stroke-dasharray": c
    });
    circle.delay(i * 150).animate(
      {
        strokeDashoffset: cbar
      },
      1000,
      "linear",
      function() {
        return circle.css({
          "transition-duration": ".3s"
        });
      }
    );
  });

  tooltipItems = $(".work li").has(".job-tooltip-trigger");
  tooltipTriggers = tooltipItems.find(".job-tooltip-trigger");
  tooltipCloseButtons = tooltipItems.find(".job-tooltip-close");
  jobTooltipMediaQuery = window.matchMedia("(max-width: 640px)");

  setJobTooltipState = function(item, isOpen) {
    var trigger;
    item.toggleClass("tooltip-open", isOpen);
    if (isOpen) {
      item.removeClass("tooltip-dismissed");
    }
    trigger = item.find(".job-tooltip-trigger").first();
    trigger.attr("aria-expanded", isOpen ? "true" : "false");
  };

  dismissJobTooltip = function(item) {
    item.removeClass("tooltip-open").addClass("tooltip-dismissed");
    return item.find(".job-tooltip-trigger").first().attr("aria-expanded", "false");
  };

  clearTooltipDismissedState = function(item) {
    return item.removeClass("tooltip-dismissed");
  };

  closeJobTooltips = function() {
    tooltipItems.each(function() {
      var item;
      item = $(this);
      item.removeClass("tooltip-dismissed");
      return setJobTooltipState(item, false);
    });
  };

  syncJobTooltips = function() {
    if (!jobTooltipMediaQuery.matches) {
      closeJobTooltips();
    }
  };

  tooltipTriggers.attr({
    "aria-haspopup": "true",
    "aria-expanded": "false"
  });

  tooltipTriggers.on("click", function(event) {
    var item, shouldOpen;
    if (!jobTooltipMediaQuery.matches) {
      return;
    }
    event.preventDefault();
    event.stopPropagation();
    item = $(this).closest("li");
    shouldOpen = !item.hasClass("tooltip-open");
    closeJobTooltips();
    return setJobTooltipState(item, shouldOpen);
  });

  tooltipTriggers.on("focus", function() {
    return clearTooltipDismissedState($(this).closest("li"));
  });

  tooltipItems.on("mouseleave", function() {
    return clearTooltipDismissedState($(this));
  });

  tooltipCloseButtons.on("click", function(event) {
    event.preventDefault();
    event.stopPropagation();
    return dismissJobTooltip($(this).closest("li"));
  });

  $(document).on("click", function(event) {
    if (!jobTooltipMediaQuery.matches) {
      return;
    }
    if ($(event.target).closest(".work li.tooltip-open").length) {
      return;
    }
    return closeJobTooltips();
  });

  $(document).on("keydown", function(event) {
    if (event.key === "Escape") {
      closeJobTooltips();
    }
  });

  if (typeof jobTooltipMediaQuery.addEventListener === "function") {
    jobTooltipMediaQuery.addEventListener("change", syncJobTooltips);
  } else if (typeof jobTooltipMediaQuery.addListener === "function") {
    jobTooltipMediaQuery.addListener(syncJobTooltips);
  }
}.call(this));

//# sourceMappingURL=data:application/json;base64,
//# sourceURL=coffeescript
